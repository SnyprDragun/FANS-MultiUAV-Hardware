...
static void GenerateTraffic(Ptr<Socket> socket, uint32_t pktSize, uint32_t pktCount, Time pktInterval)
{
  ...
  {   
    ...
    */
    ...
    //new code with ros
    std::cout<<current_state_takeoff.mode<<std::endl;
    if(current_state_takeoff.mode == "STABILIZE"){
      msg = "Stabilized mode";
    }
    else{
      msg = "Not connected";
...
int main (int argc, char *argv[])
{
  ...
  ros::init(argc, argv, "drone_state_subscriber"); // Initialize ROS node
  ...
  std::string server ("10.1.1.1"); //sender
  std::string netmask ("255.255.255.0");
  std::string macClient ("e8:4e:06:32:22:99");
  std::string macServer ("e8:4e:06:32:28:dd"); //sender
  ...
  Simulator::Schedule(Seconds(5.0), &GenerateTraffic, source, packetSize, pktcount, interPacketInterval);
