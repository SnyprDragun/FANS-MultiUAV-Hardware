...
void ReceivePacket(Ptr<Socket> socket)
 {
    ...
    while (ns3::Ptr<ns3::Packet> msg = socket->Recv())
    {
                ...
                std::cout<<"ReceivedMessage: "<<receivedData<<std::endl;
...
void ReceivePacket(Ptr<Socket> socket)
{
    ...
    while (Ptr<Packet> msg = socket->Recv())
    {
        ...
        msg->CopyData(reinterpret_cast<uint8_t*>(buffer), size);
        buffer[size] = '\0'; // Null terminate the string
        receivedData.assign(buffer, size);
